...
for i, folder_path in enumerate([f"{directory}/sfge_time", f"{directory}/deterministic_time", f"{directory}/stochastic_time"]):
...
for instance_name, instance_group in grouped_instances:
    ...
    unique_budgets = instance_group['budget'].unique()
    plt.xticks(unique_budgets)
    plt.savefig(f'plots/industry_tuning/time_budget_instance_{instance_name}')
    ...
    legend_fig.savefig(f'plots/industry_tuning/time_budget_legend.png')