import json
from classes.classes import CustomDataset
...
class Problem:
    def __init__(self, problem, instance_name, noise_factor=0, penalty=0, penalty_divide=True):
        ...
        if self.problem == "RCPSP_penalty":
            from problems.RCPSP_penalty.simulator.classes import ProductionPlan

            self.plan = ProductionPlan(**json.load(open(f'problems/RCPSP_penalty/instances/instance_{instance_name}.json')))
        ...
        for s in self.scenarios_train:
            ...
            for product in scenario_1.production_plan.products:
                for activity in product.activities:
                    ...
                    durations.append(max(1, round(sample)))
    ...
    def evaluate_mu(self, mu, data):
        y_hat = [max(int(round(x)), 1) for x in mu.tolist()]
