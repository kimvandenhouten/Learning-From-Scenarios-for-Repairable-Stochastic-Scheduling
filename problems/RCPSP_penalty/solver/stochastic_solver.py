...
def solve_rcpsp_penalty_stochastic(scenarios, durations, num_tasks, num_resources, successors, temporal_relations, demands, capacity,
    ...
    for omega in scenarios:
        ...
        mdl.add(
            start_before_start(tasks[t], tasks[s - 1], delay=temporal_relations[(t + 1, s)]) for t in range(num_tasks)
    ...
    #res = mdl.solve(TimeLimit=time_limit, Workers=1, RandomSeed=seed)
    data = []
    if res:
