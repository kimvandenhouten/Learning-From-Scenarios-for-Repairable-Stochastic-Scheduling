...
import pandas as pd
import numpy as np
from problems.RCPSP_benchmark.simulator.distributions import get_distribution, Distribution

"""
These classes are used in the Simulator. This simulator is created for a biomanufacturing scheduling application based 
on our industrial partner
"""
class Activity:
    def __init__(self, id, processing_time, product, product_id, needs, distribution=None, sequence_id=int(), constraints=[]):
    ...
    def sample_and_set_scenario(self, noise_factor=0):
        noise_mean = 0  # Mean of the noise
        noise_std_dev = 1.0 * noise_factor  # Larger standard deviation for larger noise
        noise = np.random.normal(noise_mean, noise_std_dev)
        sample = self.sample_processing_time()
        sample = max(0, round(sample + noise))
        ...
        if relations:
            for relation in relations:
                if (isinstance(relation, dict)):
                    temporal_relations[(relation['predecessor'], relation['successor'])] = relation['rel']
...
class ProductionPlan:
    ...
    def create_scenario(self, seed=None, noise_factor=0):
        plan = copy.deepcopy(self)
        if seed is not None:
            np.random.seed(seed)
        for product in plan.products:
            for activity in product.activities:
                activity.sample_and_set_scenario(noise_factor=noise_factor)
