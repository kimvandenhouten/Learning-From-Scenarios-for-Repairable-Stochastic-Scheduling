...
"""
This operator is used in the Simulator to send next activities to the factory. This simulator is created for a
biomanufacturing scheduling application based on our industrial partner
"""
...
class Operator:
    ...
    def signal_failed_activity(self, product_id, activity_id, current_time):
        ...
        if self.policy_type == 1:
            ...
            self.plan.earliest_start.append({'product_id': product_id, "activity_id": activity_id, "earliest_start": round(current_time + 1)})
    ...
    def send_next_activity(self, current_time):
        ...
        if len(self.plan.earliest_start) == 0:
            ...
            #random.shuffle(self.plan.earliest_start)
            ...
            if earliest_start == current_time:
                ...
                proc_time = max(self.plan.products[product_id].activities[activity_id].processing_time[0], 0)
